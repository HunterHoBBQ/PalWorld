<script>
	import { Heading, P, Button } from 'flowbite-svelte';
</script>

<section
	class="pt-12 bg-center bg-no-repeat bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/conference.jpg')] bg-gray-700 bg-blend-multiply"
>
	<div class="px-4 mx-auto max-w-screen-xl text-center py-24 lg:py-56">
		<h1
			class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-white md:text-5xl lg:text-6xl"
		>
			Pals World<br /> (Survivor & Pals)
		</h1>

		<p class="mb-8 text-lg font-normal text-gray-300 lg:text-xl sm:px-16 lg:px-48">
			Here at Pals World we focus on markets where technology, innovation, and capital can unlock
			long-term value and drive economic growth.
		</p>
		<div class="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
			<a
				target="_blank"
				href="https://grabjobs.co/malaysia/job/full-time/customer-service-guest-services/player-support-specialist-gaming-20665569"
				class="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-900"
			>
				Apply Now
				<svg
					aria-hidden="true"
					class="ml-2 -mr-1 w-4 h-4"
					fill="currentColor"
					viewBox="0 0 20 20"
					xmlns="http://www.w3.org/2000/svg"
					><path
						fill-rule="evenodd"
						d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
						clip-rule="evenodd"
					/></svg
				>
			</a>
			<!-- Extra button -->
			<!-- <a href="#" class="inline-flex justify-center hover:text-gray-900 items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg border border-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-400">
                Learn more
            </a>   -->
		</div>
	</div>
</section>

<div class="text-center mt-10 mb-10 z-10">
	<Heading tag="h1" class="mb-4" customSize="text-4xl font-extrabold md:text-5xl lg:text-6xl">
		Newest Survivor World
		<!-- Newest MetaWorld -->
	</Heading>
	<P class="z-20 mb-6 text-center text-lg lg:text-xl sm:px-16 xl:px-48 dark:text-gray-400">
		This is the best next big game / 这是最好的下一个大型手机游戏</P
	>
	<Button href="/">Download & Play Now!</Button>
</div>
